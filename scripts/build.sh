...
python3 "$ROOT_DIR/scripts/generate_relocation_manifest.py" \
    --readelf "$LLVM_BIN/llvm-readelf" \
    --object "$BUILD_DIR/a16_pudding_sampling_rate_handler.o" \
    --section .rela.a16_pointer_rate_worker \
    --prefix A16_POINTER_RATE_WORKER \
    --output "$BUILD_DIR/a16_pointer_rate_worker.relocations.conf" \
    --expect PUDDING_WORKER_STANDBY=R_AARCH64_CALL26 \
    --expect PUDDING_WORKER_RETURN=R_AARCH64_JUMP26
"$CLANG" --target=aarch64-linux-android35 -c \
...
python3 "$ROOT_DIR/scripts/generate_relocation_manifest.py" \
    ...
    --object "$BUILD_DIR/a16_shifted_pointer_rate_handler.o" \
    --section .rela.a16_pointer_rate_worker \
    --prefix A16_SHIFTED_POINTER_RATE_WORKER \
    --output "$BUILD_DIR/a16_shifted_pointer_rate_worker.relocations.conf" \
    --expect PUDDING_WORKER_STANDBY=R_AARCH64_CALL26 \
    --expect PUDDING_WORKER_RETURN=R_AARCH64_JUMP26
"$CLANG" --target=aarch64-linux-android35 -c \
...
"$LLVM_BIN/llvm-objcopy" \
    ...
    --dump-section .a16_pointer_rate_worker="$BUILD_DIR/a16_pudding_rate_worker.template.bin" \
...
"$LLVM_BIN/llvm-objcopy" \
    ...
    --dump-section .a16_pointer_rate_worker="$BUILD_DIR/a16_shifted_pointer_rate_worker.template.bin" \
...
require_size "$BUILD_DIR/a16_pudding_rate_worker.template.bin" 128
require_size "$BUILD_DIR/a16_shifted_pointer_rate_worker.template.bin" 128
...
python3 "$ROOT_DIR/tests/a16_pointer_rate_payload_contract.py" \
    ...
    "$BUILD_DIR/a16_pudding_rate_worker.template.bin" \
    "$BUILD_DIR/a16_pointer_rate_worker.relocations.conf"
